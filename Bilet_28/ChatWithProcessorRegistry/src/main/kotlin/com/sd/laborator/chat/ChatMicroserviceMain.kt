...
// SRP: primeste si afiseaza mesajele (text + notificari de sistem)
        ...
        launch(Dispatchers.IO) {
            try {
                while (true) {
                    ...
                    when (parts[0]) {
                        "MESSAGE" -> println("[$myName] <<< ${parts.getOrElse(1){"?"}}: ${parts.getOrElse(2){""}}")
                        // Notificari de sistem despre procesoare (trimise de MessageManager)
                        "SYSTEM"  -> println("[$myName] *** SISTEM: ${parts.drop(1).joinToString(" ")}")
                    }
                }
            } catch (_: Exception) { }
            println("[$myName] Conexiune pierduta.")
        }
        println("[$myName] Pornit, ascult mesaje")
        // Tine procesul viu
