...
// OCP: logica de stocare abonati e in MessageRouterServiceImpl; pot schimba fara sa ating serverul
// DIP: serverul depinde de IMessageRouterService (abstractie), nu de implementarea concreta
class MessageRouterServiceImpl : IMessageRouterService {
    private val subs = ConcurrentHashMap<Int, Subscriber>()

    override fun subscribe(id: Int, name: String, socket: Socket) {
        subs[id] = Subscriber(id, name, socket)
    }
    ...
    override fun getAll(): List<Subscriber> = subs.values.toList()
}
...
class MessageManagerMicroservice(private val router: IMessageRouterService) {
    ...
    private suspend fun handleClient(client: Socket) = withContext(Dispatchers.IO) {
        ...
        println("[MessageManager] Conectat: ${client.inetAddress.hostAddress}:$id")
        try {
            while (true) {
                val line = reader.readLine() ?: break
                println("[MessageManager] Primit: $line")
