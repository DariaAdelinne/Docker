...
class MulticastProcessorServiceImpl : IMulticastProcessorService {
    ...
    override fun listUsers(): Map<String, UserEndpoint> = users.toMap()
    ...
    private suspend fun handleClient(client: Socket) = withContext(Dispatchers.IO) {
        client.use { s ->
            ...
            val response: String = when (parts[0].uppercase()) {
                ...
                // LIST_USERS — folosit de WebController pentru a afisa toti utilizatorii
                "LIST_USERS" -> {
                    val users = service.listUsers()
                    if (users.isEmpty()) "USERS (none)"
                    else "USERS " + users.values.joinToString("|") { "${it.name}:${it.host}:${it.port}" }
                }
                // LIST_GROUPS
                "LIST_GROUPS" -> {
